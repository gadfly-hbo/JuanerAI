...
- **AC-MA-008-01:** Every non-deferred obligation in the fourteen-canary matrix in `test-plan.md` passes against the exact Candidate and installed host configuration without a production-valid DISPATCH; each evidence record names subject SHA, host, command definition hash, sanitized result hash, and time. No new Canary interface, direct production-adapter invocation, or disposable Change is permitted.

...
Validator 008 repeated independent verification from a clean committed Head and
passed the standard matrix, reconstructed 84/84 equivalent leaves, Unicode
17/17 leaves, scope, traceability, retirement, and repository-cleanliness
checks. Controller acceptance is PASS and archive is authorized.
External review of PR #14 then found two additional frozen-contract defect
families: percent-encoded MP9 `file:` URI bypasses and inconsistent Unicode
scalar identity length between manifest admission and Runtime construction.
Validator 008 and Controller acceptance remain historical but no longer release
the candidate. Archive is withdrawn, production is frozen, and formal Test
Design is active without a Spec semantic or scope change.
Formal Test added the URI and Unicode-scalar boundaries only to the two existing
contract suites. Controller confirmed package 282/236/46, Runtime 98/94/4, and
E 380/330/50 with failures exactly on 39 new causal leaves and 11 table/aggregate
parents; inactive 15/15 and typecheck pass. Renewed TDD_READY releases only the
package and Runtime production files; both Tests and every other path are frozen.
The bounded Worker changed only those two production files. Controller required
the Runtime correction to preserve every pre-existing rejection predicate and
change only Unicode-scalar length measurement. Package 282/282, Runtime 98/98,
E 380/380, inactive 15/15, Local Analysis 198/198 and 292/292, E2E 133 PASS plus
...
External re-review then invalidated Validator 009 with one frozen-contract
equivalence defect. Controller reproduced that manifest Runtime and dependency
identities accept `"runtime identity"`, `"."`, and `".."`, while factory
Runtime, Adapter, and dependency positions reject the same values. The current
return is Test Design with production frozen: formal Test owns only the two
existing contract suites, causal RED must be Manifest-only, and the Runtime
predicate must remain unchanged. TASK-011 is reopened for fresh Validator 010;
TASK-012 and every downstream action remain blocked.

Formal Test completed the two-suite return without changing any other path.
Package 290/283/seven and complete E 400/393/seven fail only on the six
Manifest Runtime/dependency internal-whitespace, `.` and `..` leaves plus the
package parent. Runtime equivalent positions pass 110/110 with zero predictor
issue; inactive 15/15 and typecheck pass. Controller accepts TDD_READY and
releases only `packages/contracts/model-pack.ts`; Runtime and both Tests are
frozen.

The bounded Worker changed only the package contract and left Runtime bytes
unchanged. Controller confirmed package 290/290, Runtime 110/110, E 400/400,
inactive 15/15, typecheck, affected Local Analysis 198/198 and 292/292, E2E
133 PASS plus one authorized skip, board 12/12, and canonical 1,344 PASS plus
that skip. The complete `+46/-0` Test delta remains two owned permanent
regressions with no temporary/orphan/retirement asset; mandatory complete-diff
ponytail is `Lean already. Ship.` GREEN and Test Asset Retirement PASS freeze
...
Fresh read-only Validator 011 passed exact clean Head
...
| REQ-MPC-001 | AC-MPC-001-02 | TEST-MPC-001 | TASK-003,006 | canonical synchronous serializer/admission; closed all-own-key calls including arrays; exact governed identity bounds and non-path semantics; Proxy sanitization; detached bytes/value | VALIDATOR 011 PASS |
| REQ-MPC-001 | AC-MPC-001-03 | TEST-MPC-001 | TASK-003,006 | exact sanitized error carrier with provenance/context ownership; synchronous contract throw; compatibility/artifact/status/permission precedence | VALIDATOR 011 PASS |
| REQ-MPC-001 | AC-MPC-001-04 | TEST-MPC-001 | TASK-003,006 | purpose/permission/runtime manifest admission including missing/extra/widened-permission precedence | VALIDATOR 011 PASS |
...
| REQ-MPC-002 | AC-MPC-002-01 | TEST-MPC-003 | TASK-003,006 | exact release serialize/admit signatures + bounded non-path identities + release bytes/observation/expected binding + closed local-URI verification assertion | VALIDATOR 011 PASS |
| REQ-MPC-002 | AC-MPC-002-02 | TEST-MPC-003 | TASK-003,006 | release-stage/decision gate | GREEN PASS |
| REQ-MPC-002 | AC-MPC-002-03 | TEST-MPC-003 | TASK-003,006 | exact Registry/URI plus supplied location-verification shape/kind admission; no path access | VALIDATOR 011 PASS |
...
| REQ-MPC-003 | AC-MPC-003-01 | TEST-MPC-005 | TASK-004,007 | exact binding/predictor factory; closed dependencies array; governed identity printable/no-whitespace/no-control/Unicode-scalar-length/no-credential/non-path boundary; adversarial carrier sanitization | VALIDATOR 011 PASS |
...
| REQ-MPC-005 | AC-MPC-005-03 | TEST-MPC-009 + Validator review | TASK-004,011,012 | no active binding in E; activation stop line | VALIDATOR 011 PASS |
| REQ-MPC-005 | AC-MPC-005-04 | TEST-MPC-001,005,007,009 | TASK-003,004,011 | supplied revoked-status block, retained admitted evidence, rollback review | VALIDATOR 011 PASS |
| REQ-MPC-005 | AC-MPC-005-05 | Controller/Validator evidence | TASK-011,012 | scope/ownership/integration-order review | VALIDATOR 011 PASS |

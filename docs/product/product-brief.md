...
JuanerAI is the commercial project family. Xanthil is its first product and is now desktop-first for macOS and Windows; continued CLI product development is paused while current CLI behavior remains a compatibility baseline. Enterprise capability remains future work. Reusable Domain Packs, Model Packs, Runtime, data, semantic, asset, and governance modules may later become independently versioned subprojects.
...
Model Pack product work starts only after the Xanthil Desktop Data Analyst required workflow completes and receives explicit user acceptance.
...
The current product candidate is indexed at `docs/planning/README.md`. It requires a fresh Development-Readiness Review and explicit user product approval. The static Demo is then evaluated; productization still requires the Desktop technology, local Python technical boundary, exact first Change, persistence/Runtime/cross-platform contracts, activation, and rollback to be frozen. No plan or Reviewer verdict starts a Change.

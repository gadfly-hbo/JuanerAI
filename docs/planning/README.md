...
package stays frozen; this binding does not restart it or change its scope.
...
and [complete original v1.3 snapshot](https://github.com/gadfly-hbo/JuanerAI/blob/c5a305795bd7201b541e5f8184554bf72ed82b35/docs/planning/2026-09-19/juanerai-product-development-blueprint-v1.3.md).
A local read is `git show <fixed-commit>:<repository-relative-path>`;
the original first-package, UI-base and technical-amendment hashes remain in
the Blueprints. Materializing those assets into main is not authorized here.

The prior readiness verdicts describe their original complete review packages,
not a claim that all attachments or then-current execution policy are copied
into this tree. Before any product dispatch, the exact applicable inputs must

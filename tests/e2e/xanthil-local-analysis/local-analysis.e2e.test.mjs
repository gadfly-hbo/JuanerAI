...
import { approvedModel, approvedQuestion, calculateFixtureOracle, canonicalFixtureBytes, fixtureByteSize, fixtureSha256, parseClosedFixture, referenceOracle, sha256 } from '../../fixtures/xanthil-local-analysis/fixture-oracle.mjs';
import {
  ...
  structuredInput,
  successfulCliResult,
} from '../../fixtures/xanthil-local-analysis/cli-profile-harness.mjs';
import { createAgentRuntimeDouble, createLocalAnalysisExecutionDouble, createRunArtifactStoreDouble } from '../../fixtures/xanthil-local-analysis/port-contracts.mjs';
...
test('TEST-XCLI-013 deterministic substitute [AC-XCLI-001-01, AC-XCLI-002-01, AC-XCLI-002-03, AC-XCLI-005-01, AC-XCLI-006-01, AC-XCLI-007-02, AC-XCLI-011-01, AC-XCLI-012-01, AC-XCLI-015-01, AC-XCLI-016-01] drives the approved interactive scenario through the CLI entry', async () => {
  ...
  const result = await requiredExport(cli, 'runXanthil')({
...
test('TASK-006 helper health constructs the frozen one-shot input and synchronous output doubles', async () => {
  ...
  assert.deepEqual(await tracked.input[Symbol.asyncIterator]().next(), approvedQuestionEvent());
...
test('TASK-006 TEST-XCLI-013 [AC-XCLI-001-01, AC-XCLI-002-01, AC-XCLI-002-03, AC-XCLI-011-01, AC-XCLI-012-01, AC-XCLI-015-01] renders the closed deterministic success journey in causal order', async () => {
  ...
  assertFrozenClosed(events[1].proposal, Object.keys(scenario.proposal));
...
for (const [event, reason] of [[frozenEvent({ type: 'eof' }), 'eof'], [frozenEvent({ type: 'interrupt' }), 'interrupted']]) {
...
function trackedInput(events, { iterator = undefined, extraSymbol = undefined } = {}) {
  let requests = 0;
  let nextCalls = 0;
  ...
  if (extraSymbol) input[extraSymbol] = true;
  return { input: Object.freeze(input), effects: () => ({ requests, nextCalls }) };
}

function assertFrozenClosed(value, keys) {
...
test('TASK-006 TEST-XCLI-015 [AC-XCLI-013-02, AC-XCLI-013-04] stops reading input when confirmation settles first', async () => {
  ...
  const result = await requiredExport(cli, 'runXanthil')({ input: input.input, output: capturedOutput().output, application: scenario.application });
...
test('TASK-008 TEST-XCLI-015 confirmation result wins an already-requested post-confirmation interrupt race', async () => {
  const cli = await loadPublicSeam('cli');
  const confirmationStarted = deferred();
  const confirmedResult = deferred();
  const postConfirmationInputRequested = deferred();
...
for (const [label, mutate] of [
  ...
  ['wrong manifest version', (result) => { result.run.schema_version = '2.0'; return result; }],
  ['in-progress manifest', (result) => { result.run.status = 'in_progress'; delete result.run.ended_at; delete result.run.evidence; return result; }],
  ['wrong manifest run ID', (result) => { result.run.run_id = 'not-a-uuidv7'; return result; }],
  ['wrong source checksum', (result) => { result.run.sources[0].sha256 = '0'.repeat(64); return result; }],
  ['wrong artifact descriptor', (result) => { result.run.artifacts[0].path = 'outputs/O-001.json'; return result; }],
  ['wrong evidence descriptor', (result) => { result.run.evidence.path = 'other-evidence.json'; return result; }],
  ['forbidden terminal detail on success', (result) => { result.run.terminal_detail = { stage: 'execution' }; return result; }],
  ['non-canonical metrics', (result) => { result.metrics.baseline.repurchase_member_rate = { numerator: 1, denominator: 2 }; return result; }],
  ['non-canonical Finding', (result) => { result.finding.finding_id = 'F-002'; return result; }],
]) invalidApplicationRunMutation(label, mutate);
...
for (const [label, makeResult] of [
  ...
  ['cancelled manifest invalid terminal detail', () => { const result = cancelledCliResult(); result.run.terminal_detail.error_code = 'CANCELLED'; return result; }],
]) invalidApplicationRunMutation(label, makeResult);

for (const [label, mutate] of [
  ['missing proposal field', (proposal) => { delete proposal.constraints; return proposal; }],
  ['extra proposal field', (proposal) => ({ ...proposal, ambient: true })],
  ['wrong proposal semantic value', (proposal) => { proposal.question = 'alternate question'; return proposal; }],
]) {
  test(`TASK-006 TEST-XCLI-009 [AC-XCLI-002-01, AC-XCLI-003-01] rejects invalid discovered proposal: ${label}`, async () => {
    const cli = await loadPublicSeam('cli');
    const proposal = mutate(structuredClone(createCliApplicationDouble().proposal));
...
function mappedApplicationError(label, phase, error, expected) {
  test(`TASK-006 TEST-XCLI-018 [AC-XCLI-010-02, AC-XCLI-013-01] maps ${label} to its exact sanitized terminal/result pair`, async () => {
    ...
    if (phase === 'start') {
      assert.deepEqual(scenario.effects, { start: [{ question: 'Do recent member operations show a problem?', source: scenario.effects.start[0]?.source }], discover: 0, confirm: [], cancel: 0 });
...
for (const stage of ['ready', 'proposal', 'awaiting_confirmation', 'progress', 'terminal']) {
  for (const [kind, writer] of [
    ...
    test(`TASK-006 TEST-XCLI-018 [AC-XCLI-001-02, AC-XCLI-013-01] stops exactly at ${stage} when writer ${kind}s`, async () => {
      ...
      const input = trackedInput([approvedQuestionEvent(), frozenEvent({ type: 'confirm' }), frozenEvent({ type: 'interrupt' })]);
      const events = [];
      const output = Object.freeze({ write(event) { events.push(event); if (event.type === stage) return writer(); return undefined; } });
      ...
      assert.equal(events.at(-1).type, stage);
      ...
      if (stage === 'proposal') assert.deepEqual(scenario.effects, { start: [{ question: 'Do recent member operations show a problem?', source: scenario.effects.start[0]?.source }], discover: 1, confirm: [], cancel: 0 });
...
test('TEST-XCLI-013 real Pi acceptance uses only the approved synthetic fixture and closed personal Profile', {
  ...
  assert.deepEqual(result.finding.limitations.length, 3);
  assert.deepEqual(events.map((event) => event.type), ['ready', 'proposal', 'awaiting_confirmation', 'progress', 'terminal']);
  assert.equal(events.at(-1).status, 'succeeded');
  assert.equal(JSON.stringify(events).includes('ORD-001'), false);

  const runDirectory = join(runRoot, result.run.run_id);
  ...
  assert.deepEqual(persistedRun, result.run);
  assert.equal(persistedRun.status, 'succeeded');
  assert.deepEqual(persistedRun.artifacts.map((asset) => asset.artifact_id), ['Q-001', 'S-001', 'O-001', 'O-002', 'DOC-SUMMARY', 'DOC-EVIDENCE']);

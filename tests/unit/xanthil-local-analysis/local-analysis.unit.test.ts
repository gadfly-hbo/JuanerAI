...
import type { createLocalAnalysisDomain } from '../../../packages/product-core/local-analysis.ts';
...
type LocalAnalysisDomain = ReturnType<typeof createLocalAnalysisDomain>;

async function domainMethod<K extends keyof LocalAnalysisDomain>(name: K): Promise<LocalAnalysisDomain[K]> {
...
function requiredRecord(value: unknown, key: string): Record<string, unknown> {
  if (!isRecord(value) || !isRecord(value[key])) throw new Error(`expected mutable test record ${key}`);
  return value[key];
}

function requiredArray(value: unknown, key: string): unknown[] {
  if (!isRecord(value) || !Array.isArray(value[key])) throw new Error(`expected mutable test array ${key}`);
  return value[key];
}

function looseProposalClone(): Record<string, unknown> {
  const value: unknown = structuredClone(expectedAnalysisProposal());
  if (!isRecord(value)) throw new Error('expected proposal test record');
  return value;
}

function artifactDescriptor(artifact_id: string, category: string, path: string, media_type: string) {
...
test('TEST-XCLI-003 [AC-XCLI-002-01, AC-XCLI-002-02, AC-XCLI-002-03] validates the exact closed Discovery Proposal without side effects', async (t) => {
  ...
  const add = (label: string, makeProposal: () => unknown, expectedCode = 'VALIDATION_FAILED') => {
  ...
  leaves.push(t.test('positive_same_reference_unmutated_unfrozen_no_default', async () => {
    ...
    const assertCallerGraphUnfrozen = (value: unknown, seen = new Set<object>()) => {
  ...
  const proposalKeys: Array<keyof ReturnType<typeof expectedAnalysisProposal>> = ['schema_version', 'original_question', 'question', 'objective', 'source_ids', 'fixture', 'time_windows', 'metrics', 'signal_rule', 'output_requirements', 'constraints'];
  ...
  const windowKeys: Array<keyof ReturnType<typeof expectedAnalysisProposal>['time_windows'][number]> = ['window_id', 'start_date', 'end_date'];
  ...
  const metricKeys: Array<keyof ReturnType<typeof expectedAnalysisProposal>['metrics'][number]> = ['metric_id', 'display_name', 'definition', 'grain', 'population', 'unit'];
  ...
  const booleanConstraintKeys: Array<Exclude<keyof ReturnType<typeof expectedAnalysisProposal>['constraints'], 'approved_tools_only'>> = ['synthetic_fixture_only', 'raw_row_model_egress', 'network_tools', 'generic_code_or_filesystem', 'decision_recommendation_or_action'];
  for (const key of booleanConstraintKeys) {
  ...
  const proposalMutations: Array<[string, (proposal: Record<string, unknown>) => void]> = [
    ['fixture_missing', (proposal) => { delete requiredRecord(proposal, 'fixture').kind; }],
    ['fixture_null', (proposal) => { requiredRecord(proposal, 'fixture').kind = null; }],
    ['fixture_extra', (proposal) => { requiredRecord(proposal, 'fixture').extra = true; }],
    ['fixture_nonplain', (proposal) => { proposal.fixture = Object.create(null); }],
    ['date_coverage_missing', (proposal) => { delete requiredRecord(requiredRecord(proposal, 'fixture'), 'date_coverage').start_date; }],
    ['date_coverage_null', (proposal) => { requiredRecord(requiredRecord(proposal, 'fixture'), 'date_coverage').start_date = null; }],
    ['date_coverage_extra', (proposal) => { requiredRecord(requiredRecord(proposal, 'fixture'), 'date_coverage').extra = true; }],
    ['date_coverage_nonplain', (proposal) => { requiredRecord(proposal, 'fixture').date_coverage = Object.create(null); }],
    ['window_missing', (proposal) => { delete requiredRecord({ entry: requiredArray(proposal, 'time_windows')[0] }, 'entry').window_id; }],
    ['window_null', (proposal) => { requiredRecord({ entry: requiredArray(proposal, 'time_windows')[0] }, 'entry').window_id = null; }],
    ['window_extra', (proposal) => { requiredRecord({ entry: requiredArray(proposal, 'time_windows')[0] }, 'entry').extra = true; }],
    ['window_nonplain', (proposal) => { requiredArray(proposal, 'time_windows')[0] = Object.create(null); }],
    ['metric_missing', (proposal) => { delete requiredRecord({ entry: requiredArray(proposal, 'metrics')[0] }, 'entry').metric_id; }],
    ['metric_null', (proposal) => { requiredRecord({ entry: requiredArray(proposal, 'metrics')[0] }, 'entry').metric_id = null; }],
    ['metric_extra', (proposal) => { requiredRecord({ entry: requiredArray(proposal, 'metrics')[0] }, 'entry').extra = true; }],
    ['metric_nonplain', (proposal) => { requiredArray(proposal, 'metrics')[0] = Object.create(null); }],
    ['signal_rule_missing', (proposal) => { delete requiredRecord(proposal, 'signal_rule').comparison; }],
    ['signal_rule_null', (proposal) => { requiredRecord(proposal, 'signal_rule').comparison = null; }],
    ['signal_rule_extra', (proposal) => { requiredRecord(proposal, 'signal_rule').extra = true; }],
    ['signal_rule_nonplain', (proposal) => { proposal.signal_rule = Object.create(null); }],
    ['output_requirements_missing', (proposal) => { delete requiredRecord(proposal, 'output_requirements').finding; }],
    ['output_requirements_null', (proposal) => { requiredRecord(proposal, 'output_requirements').finding = null; }],
    ['output_requirements_extra', (proposal) => { requiredRecord(proposal, 'output_requirements').extra = true; }],
    ['output_requirements_nonplain', (proposal) => { proposal.output_requirements = Object.create(null); }],
    ['constraints_missing', (proposal) => { delete requiredRecord(proposal, 'constraints').synthetic_fixture_only; }],
    ['constraints_null', (proposal) => { requiredRecord(proposal, 'constraints').synthetic_fixture_only = null; }],
    ['constraints_extra', (proposal) => { requiredRecord(proposal, 'constraints').extra = true; }],
    ['constraints_nonplain', (proposal) => { proposal.constraints = Object.create(null); }],
  ];
  for (const [family, mutate] of proposalMutations) add(`nested_${family}`, () => {
    const proposal = looseProposalClone();
    mutate(proposal);
    return proposal;
  });
...
test('TEST-XCLI-002 [AC-XCLI-005-02, AC-XCLI-005-03, AC-XCLI-011-01, AC-XCLI-011-03, AC-XCLI-012-01] permits only the bounded supported Finding', async (t) => {
  ...
  ] satisfies ReadonlyArray<readonly [string, unknown, unknown]>) await t.test(label, () => {
...
test('TEST-XCLI-004 [AC-XCLI-003-01, AC-XCLI-009-01, AC-XCLI-010-01, AC-XCLI-010-02, AC-XCLI-016-02] rejects non-closed lifecycle records and unknown versions', async (t) => {
  ...
  ] satisfies ReadonlyArray<readonly [string, unknown]>) await t.test(label, () => {
...
test('TEST-XCLI-017 [AC-XCLI-011-02, AC-XCLI-012-02, AC-XCLI-015-01, AC-XCLI-015-02] reproduces the result from recorded assets, not session history', async (t) => {
  ...
  ] satisfies ReadonlyArray<readonly [string, Record<string, unknown>]>) await t.test(label, () => {
    assert.throws(

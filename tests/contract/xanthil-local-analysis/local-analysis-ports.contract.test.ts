...
import type { AgentAnalysisRuntime } from '../../../packages/ports/local-analysis.ts';
import {
  approvedModel,
...
] satisfies readonly { label: string; exportName: keyof PortModule; testId: string; methods: readonly string[]; leaks: readonly string[] }[]);

type TestRecord = Record<string, unknown>;
type TestEvent = TestRecord & { type: string };
type TestCallback = (...args: unknown[]) => unknown;
type TestScenario = ReturnType<typeof createPiAdapterScenario>;
type TestAdmission = { tool_name: string; input: unknown };
type PortModule = typeof import('../../../packages/ports/local-analysis.ts');
...
function implementationWith(methods: readonly string[]) {
...
type AgentOpenInput = Parameters<AgentAnalysisRuntime['openSession']>[0];

const r3OpenInput = (execution_tools: AgentOpenInput['execution_tools']): AgentOpenInput => Object.freeze({
...
function r3ExecutionTools(): AgentOpenInput['execution_tools'] {
...
test('TASK-010 R3 TEST-XCLI-006 [AC-XCLI-001-02, AC-XCLI-007-01, AC-XCLI-007-03, AC-XCLI-007-04] exposes exactly preflightModel plus openSession and rejects session opening before successful preflight', async () => {
  ...
  await assert.rejects(
    async () => runtime.openSession(r3OpenInput(r3ExecutionTools())),
...
for (const [label, input, code] of [
  ...
  test(`TASK-010 R3 TEST-XCLI-011 [AC-XCLI-001-02, AC-XCLI-007-03] rejects ${label} preflight input before Session construction`, async () => {
    ...
    const adapter = await loadPublicSeam('agentAdapter');
    const runtime = requiredExport(adapter, 'createPiAgentAnalysisRuntime')(r3Model, { sdkSessionFactory: control.sdkSessionFactory });
    await assert.rejects(() => invokeNegativeOperationalPort(runtime.preflightModel, [input]), new RegExp(code));
...
test('TASK-010 R3 TEST-XCLI-011 [AC-XCLI-007-03, AC-XCLI-007-04, AC-XCLI-014-01, AC-XCLI-014-02] production construction orders local-only readiness before the one inert Session and reads Pi actual state', async () => {
  const source = await readFile(new URL('../../../adapters/agent-pi/local-analysis.ts', import.meta.url), 'utf8');
...
async function loadPortDefinition(exportName: keyof PortModule) {
...
for (const definition of portDefinitions) {
  test(`TASK-010 R3 ${definition.testId} ${definition.label} accepts and freezes the exact preflight-plus-business methods`, async () => {
    ...
    const port = requiredRecord(result, 'defined Port');
    for (const method of definition.methods) assert.equal(typeof port[method], 'function', method);
  });
...
test('TASK-005 helper health: deterministic mutation scenario completes an offline full Discovery/Execution lifecycle', async () => {
  ...
  const events: string[] = [];
  const facadeRecord = requiredRecord(facade, 'SDK facade');
  const subscribe = facadeRecord.subscribe;
  if (typeof subscribe !== 'function') throw new Error('SDK facade subscribe must be callable');
  const unsubscribe = subscribe((event: unknown) => {
    const type = requiredRecord(event, 'SDK event').type;
    if (typeof type !== 'string') throw new Error('SDK event type must be a string');
    events.push(type);
  });
  ...
  assert.deepEqual(facade.getActualModel(), { provider: 'minimax-cn', model_id: 'MiniMax-M3' });
  if (typeof unsubscribe !== 'function') throw new Error('SDK unsubscribe must be callable');
  unsubscribe();
...
test('TASK-005 TEST-XCLI-006 [AC-XCLI-006-01, AC-XCLI-006-02, AC-XCLI-006-03, AC-XCLI-007-01, AC-XCLI-007-02, AC-XCLI-007-03, AC-XCLI-013-02, AC-XCLI-013-03] drives the Pi Adapter through the frozen offline SDK construction seam', async () => {
  ...
  for (const tool of request.custom_tools) {
    ...
    assert.equal(requiredRecord(tool.parameters, 'custom tool parameters').additionalProperties, false);
  }
...
function expectedExecutionPrompt(contract: unknown) {
...
async function openR4TransportSession(factoryOptions: Parameters<typeof createDeterministicSdkSessionFactory>[0] = {}) {
  const adapter = await loadPublicSeam('agentAdapter');
...
for (const [label, discovery_context] of invalidDiscoveryContexts) {
  test(`TASK-009 TEST-XCLI-006 rejects ${label} before a Discovery prompt`, async () => {
    ...
    await assert.rejects(() => invokeNegativeOperationalPort(session.discover, [input]), (error) => assertExactSanitizedError(error, 'PROTOCOL_FAILURE'));
...
test('TASK-009 TEST-XCLI-006 rejects the former vague-question Discovery shape before a prompt', async () => {
  const { session, facadeControl } = await promptContextSession();
  await assert.rejects(() => invokeNegativeOperationalPort(session.discover, [Object.freeze({ question: discoveryQuestion })]), (error) => assertExactSanitizedError(error, 'PROTOCOL_FAILURE'));
  assert.equal(facadeControl.calls.filter(({ method }) => method === 'prompt').length, 0);
});
...
for (const [label, finding_context] of invalidFindingContexts) {
  test(`TASK-009 TEST-XCLI-006 rejects ${label} before an Execution prompt`, async () => {
    ...
    await assert.rejects(() => invokeNegativeOperationalPort(session.execute, [input]), (error) => assertExactSanitizedError(error, 'PROTOCOL_FAILURE'));
    assert.equal(facadeControl.calls.filter(({ method }) => method === 'prompt').length, 1);
  });
}
...
function mutableClone(value: unknown): TestRecord {
  return requiredRecord(structuredClone(value), 'mutable test clone');
}

function mutatedDiscoveryContext(mutator: (value: TestRecord) => void) {
  const value = mutableClone(expectedDiscoveryContext());
  mutator(value);
  return freezeDeep(value);
}

function mutatedFindingContext(mutator: (value: TestRecord) => void) {
  const value = mutableClone(expectedFindingContext());
  mutator(value);
  return freezeDeep(value);
}

const nestedDiscoveryContextMutations = Object.freeze([
  ['source unknown secret-like field', mutatedDiscoveryContext((value) => { requiredRecord(value.source, 'source').unapproved_secret_marker = 'do-not-log'; })],
  ['source unsafe absolute path', mutatedDiscoveryContext((value) => { requiredRecord(value.source, 'source').path = '/outside/member-orders-v1.csv'; })],
  ['source traversal path', mutatedDiscoveryContext((value) => { requiredRecord(value.source, 'source').path = '../member-orders-v1.csv'; })],
  ['source lexically invalid hash', mutatedDiscoveryContext((value) => { requiredRecord(value.source, 'source').sha256 = 'not-a-lowercase-64-hex-sha256'; })],
  ['comparison unknown field', mutatedDiscoveryContext((value) => { requiredRecord(value.comparison, 'comparison').extra = true; })],
  ['comparison signal-rule wrong shape', mutatedDiscoveryContext((value) => { requiredRecord(value.comparison, 'comparison').signal_rule = []; })],
  ['delivery output-requirements unknown field', mutatedDiscoveryContext((value) => { requiredRecord(requiredRecord(value.delivery, 'delivery').output_requirements, 'output requirements').extra = true; })],
  ['delivery constraints missing field', mutatedDiscoveryContext((value) => { delete requiredRecord(requiredRecord(value.delivery, 'delivery').constraints, 'constraints').network_tools; })],
  ['delivery proposal field order reordered', mutatedDiscoveryContext((value) => { requiredArray(requiredRecord(value.delivery, 'delivery').proposal_field_order, 'proposal field order').reverse(); })],
]);
...
for (const [label, discovery_context] of nestedDiscoveryContextMutations) {
  test(`TASK-009 TEST-XCLI-006 rejects nested Discovery context ${label} before egress`, async () => {
    const { session, facadeControl } = await promptContextSession();
    await assert.rejects(() => invokeNegativeOperationalPort(session.discover, [Object.freeze({ discovery_context })]), (error) => assertExactSanitizedError(error, 'PROTOCOL_FAILURE'));
    assert.equal(facadeControl.calls.filter(({ method }) => method === 'prompt').length, 0);
  });
}

const nestedFindingContextMutations = Object.freeze([
  ['identity unknown field', mutatedFindingContext((value) => { requiredRecord(value.identity, 'identity').extra = true; })],
  ['identity malformed finding ID', mutatedFindingContext((value) => { requiredRecord(value.identity, 'identity').finding_id = 'finding-001'; })],
  ['identity duplicate evidence ID', mutatedFindingContext((value) => { requiredRecord(value.identity, 'identity').evidence_ids = ['E-001', 'E-001']; })],
  ['interpretation unknown field', mutatedFindingContext((value) => { requiredRecord(value.interpretation, 'interpretation').extra = true; })],
  ['interpretation missing limitations', mutatedFindingContext((value) => { delete requiredRecord(value.interpretation, 'interpretation').required_limitations; })],
]);
...
async function injectedPiRuntime(factory: unknown) {
  const adapter = await loadPublicSeam('agentAdapter');
...
function mutationOpenInput(admissions: TestAdmission[], scenario?: TestScenario) {
  return Object.freeze({
    model: Object.freeze({ provider: 'minimax-cn', model_id: 'MiniMax-M3' }),
    discovery_tools: Object.freeze([]),
...
function assertExactSanitizedError(error: unknown, code: string) {
  const details = isRecord(error) ? error : {};
  assert.equal(details.code, code);
  assert.equal(/credential|raw-secret|forbidden-sdk-path|provider|sdk|node_modules|\/Users\//i.test(`${details.message ?? ''}\n${details.stack ?? ''}`), false);
  return true;
}

function assertEffectCounts(scenario: TestScenario, expected: TestRecord) {
...
function approvedExecutionInput(overrides: TestRecord = {}) {
  return Object.freeze({
...
] satisfies readonly (readonly [string, string, string, TestRecord])[]);
...
const invalidExecutionInputs = Object.freeze([
  ['missing Execution deadline', (() => { const value: TestRecord = { ...approvedExecutionInput() }; delete value.deadline_seconds; return value; })()],
...
async function temporaryRoot(t: { after(callback: () => unknown): unknown }, prefix: string) {
...
] satisfies readonly { seam: Parameters<typeof loadPublicSeam>[0]; factory: string; key: string; testId: string; methods: readonly string[] }[]);
...
for (const definition of adapterFactories) {
  test(`TASK-004 ${definition.testId} ${definition.factory} is the exact concrete Adapter export and returns only Port methods`, async (t) => {
    ...
    if (definition.seam === 'analysisAdapter') {
      const module = await loadPublicSeam('analysisAdapter');
      assert.deepEqual(Object.keys(module), [definition.factory]);
      const adapter = requiredExport(module, 'createDuckDbPythonLocalAnalysisExecution')({ workspaceRoot: root });
      assert.deepEqual(Object.keys(adapter).sort(), [...definition.methods].sort());
      const adapterRecord = requiredRecord(adapter, 'analysis Adapter');
      for (const method of definition.methods) assert.equal(typeof adapterRecord[method], 'function', method);
      return;
    }
    const module = await loadPublicSeam('artifactAdapter');
    assert.deepEqual(Object.keys(module), [definition.factory]);
    const adapter = requiredExport(module, 'createLocalRunArtifactStore')({ runRoot: root });
    assert.deepEqual(Object.keys(adapter).sort(), [...definition.methods].sort());
    const adapterRecord = requiredRecord(adapter, 'artifact Adapter');
    for (const method of definition.methods) assert.equal(typeof adapterRecord[method], 'function', method);
  });
  ...
  for (const invalidCase of ['missing', 'null', 'relative', 'unknown-field', 'non-directory', 'symlink-root', 'unsafe-root']) {
    test(`TASK-004 ${definition.testId} ${definition.factory} rejects ${invalidCase} config before an Adapter effect`, async (t) => {
      ...
      if (definition.seam === 'analysisAdapter') {
        const module = await loadPublicSeam('analysisAdapter');
        assert.throws(() => requiredExport(module, 'createDuckDbPythonLocalAnalysisExecution')(config), /SOURCE_BOUNDARY_VIOLATION|ARTIFACT_WRITE_FAILED|VALIDATION_FAILED/);
      } else {
        const module = await loadPublicSeam('artifactAdapter');
        assert.throws(() => requiredExport(module, 'createLocalRunArtifactStore')(config), /SOURCE_BOUNDARY_VIOLATION|ARTIFACT_WRITE_FAILED|VALIDATION_FAILED/);
      }
    });
  }
}

...
function exactVersion(output, label, expected) {
...
function atLeastVersion(output, label, minimum) {
  const match = output.trim().match(/(?:^|[^0-9])(\d+)\.(\d+)\.(\d+)(?:\s|$)/);
  assert.ok(match, `${label} must report a semantic version`);
...
function assertApprovedRootManifest(manifest) {
  assert.deepEqual(manifest, approvedRootManifest, 'root package.json must be the approved closed object');
}

function assertApprovedLock(lock, manifest) {
  assert.equal(typeof lock, 'object');
  assert.ok(lock && typeof lock.packages === 'object', 'npm lock must contain package entries');
  assert.deepEqual(lock.packages['']?.dependencies, manifest.dependencies, 'lock root must mirror exact direct dependencies');
  assert.equal(lock.packages['node_modules/@earendil-works/pi-coding-agent']?.version, '0.84.2');
  assert.equal(lock.packages['node_modules/typebox']?.version, '1.3.7');
}

function assertProjectLocalResolution(resolved, packageName) {
...
async function findRepositoryConfigurationFiles(root) {
...
function controlledClock(iso = '2026-08-20T00:00:00.000Z') {
  ...
  clock.advance = (delta) => { milliseconds += delta; };
  return clock;
}
...
function createVirtualDeadlineScheduler(events) {
  const scheduled = [];
...
function assertLogicalTimeout(observation) {
  ...
  assert.match(String(observation.error?.code ?? observation.error?.message), /TIMEOUT/);
}
...
function noExecutionSideEffects(events) {
...
async function createApplication({
  ...
  const dependencies = { agentRuntime, localAnalysisExecution, runArtifactStore, model, clock, deadlineScheduler };
...
for (const [label, createInvalidScheduler] of [
  ...
  ['symbol_field', () => { let calls = 0; const value = { schedule() { calls += 1; } }; value[Symbol('inspect')] = true; return { deadlineScheduler: Object.freeze(value), scheduleCalls: () => calls }; }],
...
function createSignalObservingArtifactStore(events) {
  const base = createRunArtifactStoreDouble({ events });
  const mutators = ['beginRun', 'commitConfirmedContract', 'appendAsset', 'replaceManifest', 'commitSuccess'];
  return Object.fromEntries([
    ['preflightRunRoot', base.preflightRunRoot],
    ...mutators.map((method) => [method, async (input) => {
      events.push({ event: `artifact.${method}.admission`, cancellation_signal: input?.cancellation_signal });
      return base[method](input);
    }]),
...
test('TASK-010 R3 TEST-XCLI-008 [AC-XCLI-013-01, AC-XCLI-013-02] sends the one live attempt AbortSignal to every Artifact mutator', async () => {
  ...
  assert.ok(admissions[0].cancellation_signal instanceof AbortSignal, 'beginRun must receive the attempt signal');
  assert.equal(admissions[0].cancellation_signal.aborted, false);
  assert.equal(admissions.every(({ cancellation_signal }) => cancellation_signal === admissions[0].cancellation_signal), true);
  assert.equal(confirmationError, undefined);
});

function createPendingArtifactStore(events, pendingMethod, deferred) {
  const base = createRunArtifactStoreDouble({ events });
  const methods = ['beginRun', 'commitConfirmedContract', 'appendAsset', 'replaceManifest', 'commitSuccess'];
  let pending = false;
  return {
    preflightRunRoot: base.preflightRunRoot,
    readTerminalRun: base.readTerminalRun,
    ...Object.fromEntries(methods.map((method) => [method, async (input) => {
      events.push({ event: `artifact.${method}.admission`, cancellation_signal: input.cancellation_signal });
      if (method === pendingMethod && !pending) {
        pending = true;
...
function createPendingCommitSuccessStore(events, deferred, settlement) {
  const base = createRunArtifactStoreDouble({ events });
  const methods = ['beginRun', 'commitConfirmedContract', 'appendAsset', 'replaceManifest'];
  return {
    preflightRunRoot: base.preflightRunRoot,
    readTerminalRun: base.readTerminalRun,
    ...Object.fromEntries(methods.map((method) => [method, async (input) => {
    ...
    async commitSuccess(input) {
      events.push({ event: 'artifact.commitSuccess.admission', cancellation_signal: input.cancellation_signal });
      deferred.started.resolve();
      await deferred.gate.promise;
...
test('TASK-010 R3 TEST-XCLI-015 [AC-XCLI-013-01, AC-XCLI-013-02] deadline atomically aborts a permanently pending Runtime and settles TIMEOUT', async () => {
  ...
  const signal = events.find(({ event }) => event === 'runtime.execute.begin').cancellation_signal;
...
for (const pendingMethod of ['beginRun', 'commitConfirmedContract', 'appendAsset', 'commitSuccess']) {
  test(`TASK-010 R3 TEST-XCLI-016 [AC-XCLI-013-01, AC-XCLI-013-02] deadline prevents late ${pendingMethod} publication and settles TIMEOUT`, async () => {
    ...
    const admissions = events.filter(({ event }) => event.endsWith('.admission'));
    const checkpoint = { ...observed };
    const abortedAtCheckpoint = admissions[0].cancellation_signal.aborted;
...
test('TASK-010 R3 TEST-XCLI-016 [AC-XCLI-013-01, AC-XCLI-013-02] deadline prevents late terminal replaceManifest publication', async () => {
  ...
  const observed = observeSettlement(confirmation);
  await deferred.started.promise;
  const signal = events.find(({ event }) => event === 'artifact.beginRun.admission').cancellation_signal;
  assert.equal(control.scheduled[0].fire(), undefined);
  await microtaskCheckpoint();
  const checkpoint = { ...observed };
  const abortedAtCheckpoint = signal.aborted;
...
test('TASK-010 R3 TEST-XCLI-009 copies the preflight source read_at byte-for-byte into the confirmed initial manifest', async () => {
  ...
  const preflight = localAnalysisExecution.preflightApprovedFixture;
  localAnalysisExecution.preflightApprovedFixture = async (input) => Object.freeze({ ...(await preflight(input)), read_at: preflightReadAt });
  ...
  const begin = events.find(({ event }) => event === 'artifact.beginRun');
  assert.equal(begin.initial_manifest.sources[0].read_at, preflightReadAt);
  assert.notEqual(begin.initial_manifest.sources[0].read_at, clock().toISOString());
});
...
for (const [label, override, expectedEvents, expectedCode] of [
  ...
  test(`TASK-010 R3 TEST-XCLI-009 rejects ${label} closed preflight result before the next capability`, async () => {
    const events = [];
    const runtime = createAgentRuntimeDouble({ events });
    const analysis = createLocalAnalysisExecutionDouble({ events });
...
test('TASK-010 R3 TEST-XCLI-009 maps post-confirm profile SOURCE_CHANGED without a Finding or success', async () => {
  const events = [];
  const analysis = createLocalAnalysisExecutionDouble({ events, onProfile: () => { throw new Error('SOURCE_CHANGED'); } });
  const preflight = analysis.preflightApprovedFixture;
  analysis.preflightApprovedFixture = async (input) => Object.freeze({ ...(await preflight(input)), read_at: '2026-01-02T03:04:05.678Z' });
  const application = await createApplication({ events, localAnalysisExecution: analysis });
  const handle = await application.start({ question: expectedAnalysisInput().question, source: expectedAnalysisInput().fixture });
  const proposal = await handle.discover();
  const result = await handle.confirm(proposal);
...
function createCommitFailureStore(events) {
  const store = createRunArtifactStoreDouble({ events });
  return {
    ...store,
    async commitSuccess() {
...
async function expectMappedFailure(promise, events, stage, errorCode) {
  let result;
  let caught;
  try {
  ...
  if (result?.run) {
    assert.equal(result.run.status, 'failed');
    assert.deepEqual(result.run.terminal_detail, { stage, error_code: errorCode });
  } else if (terminal) {
    assert.deepEqual(terminal.terminal_detail, { stage, error_code: errorCode });
  } else {
    assert.match(String(caught?.message), new RegExp(`${stage}.*${errorCode}|${errorCode}.*${stage}`));
  }
}
...
async function expectCancelled(promise) {
  try {
    ...
    assert.equal(result?.run?.status ?? result?.status, 'cancelled');
  } catch (error) {
    assert.match(String(error?.message), /CANCELLED/);
  }
}

function assertCancelledTerminal(events, stage) {
  const terminals = events.filter(({ event, status }) => event === 'artifact.replaceManifest' && status === 'cancelled');
  assert.equal(terminals.length, 1);
  assert.deepEqual(terminals[0].terminal_detail, { stage });
  assert.equal(Object.hasOwn(terminals[0].terminal_detail, 'error_code'), false);
  assert.equal(Object.hasOwn(terminals[0].manifest, 'evidence'), false);
  assert.equal(Object.hasOwn(terminals[0].manifest, 'success'), false);
  assert.equal(events.some(({ event }) => event === 'artifact.commitSuccess'), false);
}
...
async function isolatedDirectory(t, prefix, childName) {
...
async function createRealAnalysis(t) {
...
async function createRealArtifactStore(t) {
...
async function beginArtifactFixture(store, run_id = '0198d943-8b71-7a11-9abc-0000000000a1', cancellation_signal = new AbortController().signal) {
...
async function confirmArtifactFixture(store, run_id = '0198d943-8b71-7a11-9abc-0000000000a1', cancellation_signal = new AbortController().signal) {
...
function analysisPortInput(overrides = {}) {
...
function assertSanitizedAdapterError(error) {
  assert.match(String(error?.code ?? error?.message), /FIXTURE_NOT_FOUND|FIXTURE_MISMATCH|SOURCE_CHANGED|SOURCE_BOUNDARY_VIOLATION|CONTRACT_VERSION_UNSUPPORTED|ANALYSIS_EXECUTION_FAILED|VALIDATION_FAILED|TIMEOUT|CANCELLED|ARTIFACT_WRITE_FAILED|RUN_COLLISION|TERMINAL_IMMUTABLE/);
  assert.equal(/stdout|stderr|duckdb|python|spawn|ENOENT|EACCES|\/private\/|\/tmp\//i.test(String(error?.stack ?? error?.message)), false);
  return true;
}
...
test('task-003 application helper health: proposal oracle and arbitrary UUIDv7 Artifact double are deterministic', async () => {
  ...
  const cancelled = events.find(({ event, status }) => event === 'artifact.replaceManifest' && status === 'cancelled');
  assert.equal(cancelled.manifest.status, 'cancelled');
  assert.deepEqual(cancelled.manifest.terminal_detail, { stage: 'analysis_python' });
  assert.equal(Object.hasOwn(cancelled.manifest.terminal_detail, 'error_code'), false);
  assert.equal(Object.hasOwn(cancelled.manifest, 'evidence'), false);
  assert.equal(Object.hasOwn(cancelled.manifest, 'success'), false);
});
...
test('TASK-003B TEST-XCLI-008 success uses a complete initial manifest and run-explicit stateless Artifact commands', async () => {
  ...
  const application = await createApplication({ events });
  const handle = await application.start({ question: expectedAnalysisInput().question, source: expectedAnalysisInput().fixture });
  const result = await handle.confirm(await handle.discover());
  const writerEvents = events.filter(({ event }) => event.startsWith('artifact.'));
  const begin = writerEvents.find(({ event }) => event === 'artifact.beginRun');
  assert.deepEqual(begin.initial_manifest, expectedArtifactRun(result.run.run_id, begin.initial_manifest.started_at).initialManifest);
  assert.equal(writerEvents.filter(({ run_id }) => run_id !== undefined).every(({ run_id }) => run_id === result.run.run_id), true);
  const success = writerEvents.find(({ event }) => event === 'artifact.commitSuccess');
  const domain = requiredExport(await loadPublicSeam('core'), 'createLocalAnalysisDomain')();
  assert.deepEqual(domain.validateRunManifest(success.next_manifest), success.next_manifest);
  assert.equal(success.next_manifest.run_id, result.run.run_id);
  assert.deepEqual(success.next_manifest.artifacts.map(({ artifact_id }) => artifact_id), ['Q-001', 'S-001', 'O-001', 'O-002', 'DOC-SUMMARY', 'DOC-EVIDENCE']);
  assert.equal(writerEvents.at(-1).event, 'artifact.commitSuccess');
});
...
test('TASK-003B TEST-XCLI-015 post-analysis failure retains exactly all four successfully appended descriptors', async () => {
  ...
  const begin = events.find(({ event }) => event === 'artifact.beginRun');
  const terminal = events.find(({ event, status }) => event === 'artifact.replaceManifest' && status === 'failed');
  assert.equal(terminal.run_id, begin.run_id);
  assert.deepEqual(Object.keys(terminal.manifest).sort(), [...Object.keys(begin.initial_manifest), 'ended_at', 'terminal_detail'].sort());
  assert.equal(terminal.manifest.run_id, begin.run_id);
  const fixture = expectedArtifactRun(begin.run_id, begin.initial_manifest.started_at);
  assert.deepEqual(terminal.manifest.artifacts, ['Q-001', 'S-001', 'O-001', 'O-002'].map((artifact_id) => fixture.descriptorById[artifact_id]));
  assert.equal(Object.hasOwn(terminal.manifest, 'evidence'), false);
});
...
test('TASK-003B TEST-XCLI-015 Python-validation cancellation retains exactly Q-001 and O-001 descriptors', async () => {
  ...
  const localAnalysisExecution = createLocalAnalysisExecutionDouble({
    events,
    onValidate: async () => { markStarted(); await release; },
  });
  const application = await createApplication({ events, localAnalysisExecution });
  const handle = await application.start({ question: expectedAnalysisInput().question, source: expectedAnalysisInput().fixture });
  const confirmation = handle.confirm(await handle.discover());
  await Promise.race([started, confirmation]);
  const cancellation = handle.cancel();
  releaseValidation();
  await cancellation;
  await expectCancelled(confirmation);
  const begin = events.find(({ event }) => event === 'artifact.beginRun');
  const terminal = events.find(({ event, status }) => event === 'artifact.replaceManifest' && status === 'cancelled');
  assert.equal(terminal.run_id, begin.run_id);
  assert.deepEqual(Object.keys(terminal.manifest).sort(), [...Object.keys(begin.initial_manifest), 'ended_at', 'terminal_detail'].sort());
  const fixture = expectedArtifactRun(begin.run_id, begin.initial_manifest.started_at);
  assert.deepEqual(terminal.manifest.artifacts, ['Q-001', 'O-001'].map((artifact_id) => fixture.descriptorById[artifact_id]));
  assert.deepEqual(events.filter(({ event }) => event === 'artifact.appendAsset').map(({ artifact_id }) => artifact_id), ['Q-001', 'O-001']);
  assert.equal(Object.hasOwn(terminal.manifest, 'evidence'), false);
  assert.equal(events.some(({ event }) => event === 'artifact.commitSuccess'), false);
});

const invalidModelDependencies = [
...
for (const [label, modelDependency] of invalidModelDependencies) {
  test(`TASK-003 TEST-XCLI-009 rejects ${label} before runtime openSession or run allocation`, async () => {
    ...
    assert.match(String(caught?.message), /MODEL_UNAVAILABLE|INVALID_MODEL_CONFIGURATION|VALIDATION_FAILED/);
...
test('TASK-009 TEST-XCLI-009 [AC-XCLI-002-01, AC-XCLI-006-01, AC-XCLI-013-02] passes the two exact deeply frozen non-output prompt contexts through the Runtime Port', async () => {
  ...
  const proposal = await handle.discover();
  await handle.confirm(proposal);
  const discovery = events.find(({ event }) => event === 'runtime.discover');
  const execution = events.find(({ event }) => event === 'runtime.execute.begin');
...
test('TASK-009 R4 TEST-XCLI-009 [AC-XCLI-007-04, R4-AC-001-03, R4-AC-004-02] records only MiniMax-M3 as actual model and run provenance; Mimo never activates as fallback', async () => {
  ...
  assert.deepEqual(events.find(({ event }) => event === 'runtime.openSession').model, { provider: 'minimax-cn', model_id: 'MiniMax-M3' });
...
async function readPiAdapterProductionSource() {
  return readFile(fileURLToPath(new URL('../../../adapters/agent-pi/local-analysis.mjs', import.meta.url)), 'utf8');
}
...
const invalidRuntimeProposals = [
  ['missing_metrics', () => { const value = cloneProposal(); delete value.metrics; return value; }],
...
const proposalRejections = [
  ['empty_confirmation', () => ''],
  ['missing_objective', () => { const value = cloneProposal(); delete value.objective; return value; }],
...
test('TASK-003 TEST-XCLI-010 observes same-session stages, exact three tools, single writer, and success-last ordering', async () => {
  ...
  const openSession = events.find(({ event }) => event === 'runtime.openSession');
  assert.deepEqual(openSession.model, approvedModel);
  assert.deepEqual(openSession.execution_tools.map(({ tool_name }) => tool_name), approvedToolNames);
  for (const descriptor of openSession.execution_tools) {
    assert.equal(Object.isFrozen(descriptor), true);
    assert.deepEqual(Object.keys(descriptor).sort(), ['invoke', 'tool_name']);
  }
  const runtimeExecution = events.find(({ event }) => event === 'runtime.execute.begin');
  ...
  const executionContract = runtimeExecution.confirmed_contract;
  ...
  assert.deepEqual(appendEvents[0].asset, { artifact_id: 'Q-001', category: 'query', path: 'queries/Q-001.sql', media_type: 'application/sql', bytes: canonicalQueryBytes });
  assert.deepEqual(appendEvents[2].asset, { artifact_id: 'S-001', category: 'script', path: 'scripts/S-001.py', media_type: 'text/plain', bytes: canonicalScriptBytes });
  assert.deepEqual(JSON.parse(Buffer.from(appendEvents[1].asset.bytes)), { ...referenceOracle, calculation_kind: 'sql' });
  assert.deepEqual(JSON.parse(Buffer.from(appendEvents[3].asset.bytes)), { ...referenceOracle, calculation_kind: 'python_validation' });
  assert.deepEqual(appendEvents.slice(1, 4).filter(({ artifact_id }) => artifact_id.startsWith('O-')).map(({ asset }) => ({ artifact_id: asset.artifact_id, category: asset.category, path: asset.path, media_type: asset.media_type })), [
  ...
  assert.equal(runtimeExecution.cancellation_signal, analyticalCalls[0].cancellation_signal);
  assert.equal(analyticalCalls[0].cancellation_signal, analyticalCalls[1].cancellation_signal);
  const contractEvent = events.find(({ event }) => event === 'artifact.commitConfirmedContract');
...
test('TASK-003 TEST-XCLI-006 rejects a late post-terminal descriptor invocation before Analysis or Artifact work', async () => {
  ...
  const agentRuntime = createAgentRuntimeDouble({
    ...
    driveTurn: async ({ execution_tools, invoke }) => {
      lateInvoke = execution_tools[2].invoke;
  ...
  await assert.rejects(() => lateInvoke({ correlation_id: 'call-004', arguments: {} }), /TOOL_POLICY_VIOLATION|PROTOCOL_FAILURE|TERMINAL_IMMUTABLE/);
...
test('TASK-003 TEST-XCLI-009 keeps the generated UUIDv7 consistent through one attempt', async () => {
  ...
  const contractEvent = events.find(({ event }) => event === 'artifact.commitConfirmedContract');
  assert.equal(contractEvent.contract.run_id, result.run.run_id);
});
...
test('TASK-010 R3 TEST-XCLI-015 cancel after run creation is idempotent and rejects late runtime output', async () => {
  ...
  const executeRelease = new Promise((resolve) => { releaseExecute = resolve; });
  const agentRuntime = createAgentRuntimeDouble({ events, onExecute: async () => { markExecuteStarted(); await executeRelease; } });
  const application = await createApplication({ events, agentRuntime });
  const handle = await application.start({ question: expectedAnalysisInput().question, source: expectedAnalysisInput().fixture });
  const confirmation = handle.confirm(await handle.discover());
  await Promise.race([executeStarted, confirmation]);
  const cancellation = handle.cancel();
  releaseExecute();
...
test('TASK-010 R3 TEST-XCLI-015 cancel during validation keeps the deadline signal live and terminalizes once', async () => {
  const events = [];
  let markValidateStarted;
  let releaseValidate;
  const validateStarted = new Promise((resolve) => { markValidateStarted = resolve; });
  const validateRelease = new Promise((resolve) => { releaseValidate = resolve; });
  const localAnalysisExecution = createLocalAnalysisExecutionDouble({
    events,
    onValidate: async () => {
      markValidateStarted();
  ...
  const handle = await application.start({ question: expectedAnalysisInput().question, source: expectedAnalysisInput().fixture });
  const confirmation = handle.confirm(await handle.discover());
  await Promise.race([validateStarted, confirmation]);
  const analyticalCalls = events.filter(({ event }) => event === 'analysis.calculateMemberRepurchaseMetrics' || event === 'analysis.validateMemberRepurchaseMetrics');
  assert.equal(analyticalCalls.length, 2);
  assert.equal(analyticalCalls[0].cancellation_signal, analyticalCalls[1].cancellation_signal);
  const cancellation = handle.cancel();
  await microtaskCheckpoint();
  assert.equal(analyticalCalls[0].cancellation_signal.aborted, false);
  releaseValidate();
  const first = await cancellation;
  await expectCancelled(confirmation);
  const second = await handle.cancel();
...
for (const pendingMethod of ['beginRun', 'commitConfirmedContract', 'appendAsset']) {
  test(`TASK-010 R3 TEST-XCLI-017 [AC-XCLI-013-01, AC-XCLI-013-02] user cancellation closes admission during pending ${pendingMethod} and leaves one cancelled terminal`, async () => {
    ...
    const confirmation = handle.confirm(await handle.discover());
    await deferred.started.promise;
    const signal = events.find(({ event }) => event.endsWith('.admission')).cancellation_signal;
    ...
    assert.equal(cancelled?.run?.status ?? cancelled?.status, 'cancelled');
...
test('TASK-010 R3 TEST-XCLI-017 [AC-XCLI-013-01, AC-XCLI-013-02] user cancellation during admitted commitSuccess converges to succeeded when final manifest linearizes first', async () => {
  ...
  const confirmation = handle.confirm(await handle.discover());
  await deferred.started.promise;
  const signal = events.find(({ event }) => event === 'artifact.commitSuccess.admission').cancellation_signal;
  const normalAdmissionsBeforeCancel = events
    .filter(({ event }) => event.endsWith('.admission') && event !== 'artifact.replaceManifest.admission')
    .map(({ event }) => event);
  const cancellation = handle.cancel();
...
for (const [label, input] of [
  ...
  test(`TASK-010 R3 TEST-XCLI-009 [AC-XCLI-001-02, AC-XCLI-007-02] rejects ${label} as a closed fixture preflight boundary`, async (t) => {
    const { workspaceRoot, execution } = await createRealAnalysis(t);
    await assert.rejects(() => execution.preflightApprovedFixture(input), /SOURCE_BOUNDARY_VIOLATION/);
    assert.deepEqual(await readdir(workspaceRoot), ['member-orders-v1.csv']);
  });
}
...
test('TASK-004 TEST-XCLI-012 validates independently with real Python and frozen canonical script', async (t) => {
  ...
  const result = await execution.validateMemberRepurchaseMetrics(analysisPortInput({ sql_result: sql.result }));
  assert.deepEqual(result, {
...
test('TASK-004 TEST-XCLI-012 real analytical envelopes leak no rows, paths, process handles, or workspace outputs', async (t) => {
  const { workspaceRoot, execution } = await createRealAnalysis(t);
  const sql = await execution.calculateMemberRepurchaseMetrics(analysisPortInput());
  const python = await execution.validateMemberRepurchaseMetrics(analysisPortInput({ sql_result: sql.result }));
...
const analysisSourceFailures = Object.freeze([
...
for (const [label, arrange] of analysisSourceFailures) {
  test(`TASK-004 TEST-XCLI-015 fails closed for ${label} before analytical output`, async (t) => {
    const context = await createRealAnalysis(t);
    const { input = analysisPortInput() } = await arrange({ ...context, input: analysisPortInput() });
    await assert.rejects(() => context.execution.calculateMemberRepurchaseMetrics(input), assertSanitizedAdapterError);
...
for (const [label, forbidden] of [
  ...
  test(`TASK-004 TEST-XCLI-015 rejects ${label} at the closed Analysis Port`, async (t) => {
    const { execution } = await createRealAnalysis(t);
    await assert.rejects(() => execution.calculateMemberRepurchaseMetrics(analysisPortInput(forbidden)), assertSanitizedAdapterError);
  });
}
...
test('TASK-004 TEST-XCLI-015 rejects an already-aborted analytical call before reading output', async (t) => {
  ...
  await assert.rejects(() => execution.validateMemberRepurchaseMetrics(analysisPortInput({ sql_result: referenceOracle, cancellation_signal: abort.signal })), assertSanitizedAdapterError);
  assert.deepEqual(await readdir(workspaceRoot), ['member-orders-v1.csv']);
});
...
test('TASK-004 TEST-XCLI-015 treats required deadline_seconds zero as an immediate sanitized TIMEOUT', async (t) => {
  ...
  await assert.rejects(
    ...
    (error) => String(error?.code ?? error?.message) === 'TIMEOUT' && assertSanitizedAdapterError(error),
  );
});
...
for (const [label, deadline] of [['missing', undefined], ['negative', -1], ['non-integer', 0.5], ['over-limit', 31]]) {
  test(`TASK-004 TEST-XCLI-015 rejects ${label} analytical deadline before execution`, async (t) => {
    ...
    await assert.rejects(() => execution.calculateMemberRepurchaseMetrics(input), assertSanitizedAdapterError);
  });
}
...
for (const [label, mutate] of [
  ...
  test(`TASK-004 TEST-XCLI-016 rejects ${label} before an Artifact file appears`, async (t) => {
    const { runRoot, store } = await createRealArtifactStore(t);
    const fixture = await confirmArtifactFixture(store);
    await assert.rejects(() => store.appendAsset(artifactMutatorInput({ run_id: fixture.run_id, asset: mutate(fixture.assets[0]) })), assertSanitizedAdapterError);
    assert.equal(existsSync(join(runRoot, fixture.run_id, fixture.assets[0].path)), false);
  });
}

for (const [label, operation] of [
  ['missing command field', (store, fixture) => store.appendAsset(artifactMutatorInput({ asset: fixture.assets[0] }))],
  ['unknown command field', (store, fixture) => store.appendAsset(artifactMutatorInput({ run_id: fixture.run_id, asset: fixture.assets[0], overwrite: true }))],
...
const closedArtifactCommands = Object.freeze([
  ['beginRun', async (store) => {
    const fixture = expectedArtifactRun('0198d943-8b71-7a11-9abc-0000000000a1');
    return { fixture, invoke: () => store.beginRun(artifactMutatorInput({ run_id: fixture.run_id, initial_manifest: fixture.initialManifest, cwd: '/ambient' })) };
  }],
  ['commitConfirmedContract', async (store) => {
    const fixture = await beginArtifactFixture(store);
    return { fixture, invoke: () => store.commitConfirmedContract(artifactMutatorInput({ run_id: fixture.run_id, contract: fixture.contract, overwrite: true })) };
  }],
  ['replaceManifest', async (store) => {
    const fixture = await beginArtifactFixture(store);
    return { fixture, invoke: () => store.replaceManifest(artifactMutatorInput({ run_id: fixture.run_id, next_manifest: fixture.failedManifest, repair: true })) };
  }],
  ['commitSuccess', async (store) => {
    ...
    return {
      fixture,
      invoke: () => store.commitSuccess(artifactMutatorInput({
  ...
  ['readTerminalRun', async (store) => {
    ...
    return { fixture, invoke: () => store.readTerminalRun({ run_id: fixture.run_id, repair: true }) };
  }],
]);
...
for (const label of ['failed', 'cancelled']) {
...
async function artifactByteSnapshot(root) {
...
async function preflightRealArtifactStore(store) {
...
test('TASK-010 R3 TEST-XCLI-008 [AC-XCLI-001-01, AC-XCLI-007-01] maps per-start missing, replaced, symlink, and non-directory run roots to RUN_ROOT_UNSAFE without writing', async (t) => {
  ...
  for (const mode of ['missing', 'replaced', 'symlink', 'non-directory']) {
    ...
    await assert.rejects(() => store.preflightRunRoot(), (error) => String(error?.code ?? error?.message) === 'RUN_ROOT_UNSAFE');
...
for (const operation of ['beginRun', 'commitConfirmedContract', 'appendAsset', 'replaceManifest', 'commitSuccess']) {
  test(`TASK-010 R3 TEST-XCLI-008 [AC-XCLI-013-04] already-aborted ${operation} produces ARTIFACT_WRITE_FAILED and no filesystem mutation`, async (t) => {
    ...
    await assert.rejects(invoke, (error) => String(error?.code ?? error?.message) === 'ARTIFACT_WRITE_FAILED');
...
for (const [label, signal] of [
  ...
  test(`TASK-010 R3 TEST-XCLI-008 [AC-XCLI-013-01] rejects ${label} cancellation_signal before every Artifact write`, async (t) => {
    ...
    for (const [operation, payload] of inputs) {
      ...
      await assert.rejects(() => store[operation](input), (error) => String(error?.code ?? error?.message) === 'ARTIFACT_WRITE_FAILED');
      assert.deepEqual(await artifactByteSnapshot(runRoot), before);
    }
  });
}
...
test('TEST-XCLI-009 [AC-XCLI-001-01, AC-XCLI-001-02, AC-XCLI-002-01, AC-XCLI-002-02, AC-XCLI-002-03, AC-XCLI-002-04, AC-XCLI-003-01, AC-XCLI-003-02] integrates preflight and explicit confirmation through the Application use case', async () => {
  ...
  ]) await assert.rejects(() => application.start({ question: expectedAnalysisInput().question, source }), error, label);
});
...
async function isolatePiCodingAgentDirectory(t, prefix) {
...
function productionDefaultExecutionTools() {
  return Object.freeze(approvedToolNames.map((tool_name) => Object.freeze({ tool_name, invoke: async () => ({ accepted: true }) })));
}

async function assertPiReadinessOnly(configRoot, baseline) {
  const files = (await readdir(configRoot)).sort();
  assert.deepEqual(files, baseline ?? ['auth.json', 'models-store.json']);
  return files;
}

async function runProductionPiPreflightFailureCase(failureCase) {
  const { stdout, stderr } = await execFileAsync(process.execPath, [
    '--import', join(repositoryRoot, 'tests/fixtures/xanthil-local-analysis/pi-sdk-failure-hook.mjs'),
    join(repositoryRoot, 'tests/fixtures/xanthil-local-analysis/pi-sdk-failure-child.mjs'),
  ], {
    cwd: repositoryRoot,
    encoding: 'utf8',
...
const invalidProductionFactoryInputs = Object.freeze([
...
const invalidProductionOpenInputs = Object.freeze([
...
test(`TASK-005 TEST-XCLI-011 production-default mutation: ${title}`, async (t) => {
  ...
  const readinessFiles = await assertPiReadinessOnly(isolatedConfigRoot);
  await assert.rejects(() => runtime.openSession(input), /MODEL_UNAVAILABLE|PROTOCOL_FAILURE|TOOL_POLICY_VIOLATION/);
  await assertPiReadinessOnly(isolatedConfigRoot, readinessFiles);
});
}
...
test('TASK-010 R3 TEST-XCLI-019 [AC-XCLI-013-01, AC-XCLI-013-02] Profile binds an ordinary one-shot host-timer deadline scheduler without composition effects', async () => {
  const source = await readFile(fileURLToPath(new URL('../../../profiles/personal/local-analysis.mjs', import.meta.url)), 'utf8');
...
async function createTask006Roots(t) {
...
function validTask006ProfileConfig({ workspaceRoot, runRoot }) {
...
test('TEST-XCLI-021 [AC-XCLI-001-01, AC-XCLI-007-01, AC-XCLI-008-01, AC-XCLI-016-01] enforces the reproducible project-local dependency and engine contract', async () => {
  ...
  assert.throws(() => assertApprovedRootManifest({ ...approvedRootManifest, devDependencies: { typescript: '5.0.0' } }));
  ...
  const lockFixture = {
    lockfileVersion: 3,
    packages: {
      '': { dependencies: structuredClone(approvedRootManifest.dependencies) },
  ...
  for (const [packageName, expectedVersion] of Object.entries(approvedRootManifest.dependencies)) {
    assertProjectLocalResolution(fileURLToPath(import.meta.resolve(packageName)), packageName);
...
test('TEST-XCLI-022 [AC-XCLI-016-01, AC-XCLI-016-04] keeps all four actual target suites in frozen .mjs paths without compiler/build artifacts', async () => {
  const targets = [
    new URL('../../unit/xanthil-local-analysis/local-analysis.unit.test.mjs', import.meta.url),
    new URL('../../contract/xanthil-local-analysis/local-analysis-ports.contract.test.mjs', import.meta.url),
    new URL('../xanthil-local-analysis/local-analysis.integration.test.mjs', import.meta.url),
    new URL('../../e2e/xanthil-local-analysis/local-analysis.e2e.test.mjs', import.meta.url),
  ];
  ...
  assert.equal(names.flat().some((name) => /(^|\/)(dist|build)|\.tsx?$|tsconfig\.json/.test(name)), false);
});

...
import { createLocalAnalysisDomain } from '../product-core/local-analysis.mjs';
import {
  defineAgentAnalysisRuntime,
  defineLocalAnalysisExecution,
  defineRunArtifactStore,
} from '../ports/local-analysis.mjs';
...
function closedObject(value, keys, code = 'VALIDATION_FAILED') {
...
function confirmedContract(proposal, run_id, confirmed_at) {
  const { fixture, ...contract } = proposal;
  return { schema_version: contract.schema_version, run_id, confirmed_at, ...contract };
}

function uuidV7(clock) {
  const date = clock();
...
function validateDeadlineScheduler(value) {
  if (value === null || typeof value !== 'object' || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype || !Object.isFrozen(value)) fail('VALIDATION_FAILED');
...
function closedFrozenResult(value, keys, code) {
  if (value === null || typeof value !== 'object' || Array.isArray(value) || Object.getPrototypeOf(value) !== Object.prototype || !Object.isFrozen(value) || Object.getOwnPropertySymbols(value).length !== 0) fail(code);
...
function validateRunRootPreflight(value) {
  closedFrozenResult(value, ['ready'], 'RUN_ROOT_UNSAFE');
  if (value.ready !== true) fail('RUN_ROOT_UNSAFE');
}

function validateFixturePreflight(value) {
  closedFrozenResult(value, ['source_id', 'kind', 'path', 'sha256', 'byte_size', 'fixture_version', 'read_at'], 'FIXTURE_MISMATCH');
  const { read_at, ...descriptor } = value;
  if (!sameValue(descriptor, sourceDescriptor) || !isValidUtcRfc3339(read_at)) fail('FIXTURE_MISMATCH');
  return value;
}

function validateModelPreflight(value) {
  closedFrozenResult(value, ['provider', 'model_id'], 'MODEL_UNAVAILABLE');
  if (!sameValue(value, modelIdentity)) fail('MODEL_UNAVAILABLE');
}

function validateDeadlineHandle(value) {
  closedFrozenResult(value, ['cancel'], 'VALIDATION_FAILED');
  if (typeof value.cancel !== 'function' || value.cancel.length !== 0) fail('VALIDATION_FAILED');
  return value;
}
...
function validateStartInput(input) {
  ...
  if (input.source.path.startsWith('/') || input.source.path.split('/').includes('..')) fail('SOURCE_BOUNDARY_VIOLATION');
...
function validateEnvelope(value, calculationKind, asset) {
  ...
  delete result.calculation_kind;
  return { result, canonical_asset: value.canonical_asset };
}

function outputAsset(artifact_id, result) {
...
function validateBeginResult(value, run_id) {
  closedObject(value, ['run_id'], 'ARTIFACT_WRITE_FAILED');
  if (value.run_id !== run_id) fail('ARTIFACT_WRITE_FAILED');
}

function validateContractCommitResult(value, run_id, contract, contractHash) {
...
function validateAppendResult(value, expected) {
...
function mappedError(error, fallbackStage, fallbackCode) {
  if (error?.stage && error?.code) return error;
...
export function createLocalAnalysisApplication(dependencies) {
  ...
  async function start(input) {
    ...
    let runtimeCancelPromise;
    let runAllocated = false;
    let admittedWork;
    const executionTools = [];
    const toolState = { index: 0, correlations: new Set(), sqlResult: undefined, artifacts: [] };

    function retainedArtifacts() {
      return manifestArtifactOrder.map((artifact_id) => toolState.artifacts.find((artifact) => artifact.artifact_id === artifact_id)).filter(Boolean);
    }
    ...
    function requireNormalAdmission() {
      requireAdmission();
      if (state === 'cancelled') throw Object.assign(new Error('CANCELLED'), { code: 'CANCELLED' });
    }

    async function awaitAdmitted(work, { normal = true, onSettled } = {}) {
    ...
    async function appendAsset(asset) {
    ...
    function elapsedExceeded(startedAt) {
    ...
    async function terminalFailure(stage, error_code) {
      ...
      return async (request) => {
        ...
        if (index === 0) {
          ...
          try {
            ...
            if (state === 'cancelled') throw Object.assign(new Error('CANCELLED'), { code: 'CANCELLED' });
            throw mappedError(error, 'source_read', String(error?.message) === 'SOURCE_INVALID' ? 'SOURCE_INVALID' : 'SOURCE_BOUNDARY_VIOLATION');
          }
        }
        if (index === 1) {
          ...
          try {
            ...
            if (state === 'cancelled') throw Object.assign(new Error('CANCELLED'), { code: 'CANCELLED' });
            throw mappedError(error, 'analysis_sql', String(error?.message) === 'TIMEOUT' ? 'TIMEOUT' : String(error?.message) === 'VALIDATION_FAILED' ? 'VALIDATION_FAILED' : String(error?.message) === 'ARTIFACT_WRITE_FAILED' ? 'ARTIFACT_WRITE_FAILED' : 'ANALYSIS_EXECUTION_FAILED');
          }
        }
        activeStage = 'analysis_python';
        try {
          const validation = await awaitAdmitted(() => dependency.localAnalysisExecution.validateMemberRepurchaseMetrics({ source, run_id: run.run_id, confirmed_contract: contract, sql_result: { ...toolState.sqlResult, calculation_kind: 'sql' }, deadline_seconds: 30, cancellation_signal: abortController.signal }));
          if (state === 'cancelled') throw Object.assign(new Error('CANCELLED'), { code: 'CANCELLED' });
          ...
          if (state === 'cancelled') throw Object.assign(new Error('CANCELLED'), { code: 'CANCELLED' });
          throw mappedError(error, 'analysis_python', String(error?.message) === 'TIMEOUT' ? 'TIMEOUT' : String(error?.message) === 'VALIDATION_FAILED' ? 'VALIDATION_FAILED' : String(error?.message) === 'ARTIFACT_WRITE_FAILED' ? 'ARTIFACT_WRITE_FAILED' : 'ANALYSIS_EXECUTION_FAILED');
        }
      };
    }
    ...
    return freeze({
      async discover() {
        if (state !== 'created') fail(state === 'cancelled' ? 'CANCELLED' : 'PROTOCOL_FAILURE');
      ...
      async confirm(candidate) {
        ...
        try {
          ...
          if (state === 'cancelled') return cancellation;
          if (!runAllocated && String(error?.message) === 'RUN_COLLISION') {
            closeDeadline();
            throw error;
          }
          const mapped = mappedError(
            error,
            activeStage,
            String(error?.message) === 'MODEL_EXECUTION_FAILED'
              ? 'MODEL_EXECUTION_FAILED'
              : activeStage === 'contract_persist' && String(error?.message) === 'ARTIFACT_WRITE_FAILED'

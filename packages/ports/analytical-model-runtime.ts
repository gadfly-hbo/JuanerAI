...
const validIdentityVersion = (candidate: unknown): candidate is IdentityVersionV1 => exactKeys(candidate, ['identity', 'version']) && typeof candidate.identity === 'string' && candidate.identity.length > 0 && candidate.identity.length <= 256 && candidate.identity.trim() === candidate.identity && !/[\s\\/@\p{C}]/u.test(candidate.identity) && candidate.identity !== '.' && candidate.identity !== '..' && !/^(latest|alias)$/i.test(candidate.identity) && typeof candidate.version === 'string' && stableVersionPattern.test(candidate.version);

...
const error = (code: Parameters<typeof modelPackError>[0]): never => { throw modelPackError(code); };
const exactKeys = (value: unknown, keys: readonly string[]): value is RecordValue => Boolean(value && typeof value === 'object' && !Array.isArray(value) && Object.keys(value as object).length === keys.length && keys.every((key) => Object.hasOwn(value as object, key)));
...
export function defineAnalyticalModelRuntime(input: Readonly<{ binding: AnalyticalModelRuntimeBindingV1; predictor: LocalCategoryDemandPredictor }>): AnalyticalModelRuntime {
  ...
  const preflight = async (candidate: AnalyticalModelPreflightInputV1): Promise<AnalyticalModelReadinessV1> => {
    try {
      ...
      const manifest = admitModelPackManifest({ manifest_bytes: candidate.manifest_bytes, artifact_observation: candidate.artifact_observation, expected_package: candidate.expected_package });
      ...
      const status = admitModelPackReleaseStatus({ release_status: candidate.release_status, expected_package: candidate.expected_package });
      if (!same(expected, candidate.expected_package)) return error('MODEL_PACK_IDENTITY_MISMATCH');
  ...
  const openRun = async (candidate: AnalyticalModelOpenRunInputV1): Promise<AnalyticalModelRun> => {
    try {
      ...
      const admittedInput = admitCategoryDemandInput({ candidate: snapshot.input, manifest: candidate.readiness.manifest });
      const actualSha = createHash('sha256').update(canonicalCategoryDemandInputBytes({ admitted_input: admittedInput, manifest: candidate.readiness.manifest })).digest('hex');
...
async function executePrediction(predictor: LocalCategoryDemandPredictor, runId: string, readiness: AnalyticalModelReadinessV1, snapshot: ConfirmedCategoryDemandSnapshotV1, signal: AbortSignal, deadlineAt: string, deadline: number): Promise<CategoryDemandForecastResultV1> {
  return new Promise<CategoryDemandForecastResultV1>((resolve, reject) => {
    ...
    const fail = (code: Parameters<typeof modelPackError>[0]): void => finish(() => { throw modelPackError(code); });
    ...
    prediction.then((candidate) => {
      ...
      try {
        const forecast = admitCategoryDemandForecast({ candidate, manifest: readiness.manifest, admitted_input: snapshot.input });
        if (signal.aborted) return fail('ANALYTICAL_MODEL_CANCELLED');
        if (Date.now() >= deadline) return fail('ANALYTICAL_MODEL_DEADLINE_EXCEEDED');

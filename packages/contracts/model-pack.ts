...
const nonempty = (value: unknown, code: ModelPackErrorCode): string => {
  ...
  return output.length > 0 && output.trim() === output && !/[\x00-\x1f\x7f]/.test(output) ? output : fail(code);
};
...
const decimalNumber = (value: string): number => Number(value);
...
const identityVersion = (candidate: unknown, code: ModelPackErrorCode): IdentityVersionV1 => {
  ...
  const identityValue = nonempty(value.identity, code);
  if (/[\\/]|latest|alias/i.test(identityValue)) return fail(code);
...
const permissions = (candidate: unknown, code: ModelPackErrorCode): ModelPackPermissionsV1 => {
  const value = record(candidate, ['data', 'network', 'external_data', 'mlflow_at_runtime', 'training_workspace_at_runtime', 'source_write', 'model_execution'], code);
...
const categories = (candidate: unknown, code: ModelPackErrorCode): readonly string[] => {
  ...
  if (new Set(values).size !== values.length || values.some((entry) => entry.normalize('NFC') !== entry || entry.length > 128 || /[\\/]/.test(entry) || entry === '.' || entry === '..')) return fail(code);
  return values;
};
...
function validateManifest(candidate: unknown): ModelPackManifestV1 {
  ...
  const p = { controller_release_decision_id: nonempty(provenance.controller_release_decision_id, 'MODEL_PACK_CONTRACT_INVALID'), released_at: instant(provenance.released_at, 'MODEL_PACK_CONTRACT_INVALID'), mlflow_experiment_id: nonempty(provenance.mlflow_experiment_id, 'MODEL_PACK_CONTRACT_INVALID'), mlflow_run_id: nonempty(provenance.mlflow_run_id, 'MODEL_PACK_CONTRACT_INVALID'), registered_model_name: nonempty(provenance.registered_model_name, 'MODEL_PACK_CONTRACT_INVALID'), registered_model_version: nonempty(provenance.registered_model_version, 'MODEL_PACK_CONTRACT_INVALID'), training_data_sha256: checksum(provenance.training_data_sha256, 'MODEL_PACK_CONTRACT_INVALID'), training_code_revision: nonempty(provenance.training_code_revision, 'MODEL_PACK_CONTRACT_INVALID'), evaluation_evidence_sha256: checksum(provenance.evaluation_evidence_sha256, 'MODEL_PACK_CONTRACT_INVALID') };
  ...
  if (decimalNumber(e.observed_order_count_relative_wape_improvement) < .05 || decimalNumber(e.observed_net_order_amount_relative_wape_improvement) < .1 || decimalNumber(e.observed_key_category_wape_regression_max_percentage_points) > 5 || decimalNumber(e.observed_interval_coverage) < .7 || decimalNumber(e.observed_interval_coverage) > .9) return fail('MODEL_PACK_CONTRACT_INVALID');
  ...
  const license = record(value.license, ['license_id', 'terms_sha256'], 'MODEL_PACK_CONTRACT_INVALID'); const licenseId = nonempty(license.license_id, 'MODEL_PACK_LICENSE_INVALID'); const termsSha = checksum(license.terms_sha256, 'MODEL_PACK_LICENSE_INVALID');
...
export function serializeModelPackManifest(manifest: ModelPackManifestV1): Uint8Array { return canonical(validateManifest(manifest)); }
...
function validateRelease(candidate: unknown): ModelPackReleaseInputV1 {
  ...
  const manifest = validateManifest(value.manifest); const controllerRelease = { decision: 'model_released' as const, decision_id: nonempty(controller.decision_id, 'MODEL_PACK_CONTRACT_INVALID'), decided_at: instant(controller.decided_at, 'MODEL_PACK_CONTRACT_INVALID'), package_identity: nonempty(controller.package_identity, 'MODEL_PACK_CONTRACT_INVALID'), package_version: stableVersion(controller.package_version, 'MODEL_PACK_CONTRACT_INVALID'), artifact_sha256: checksum(controller.artifact_sha256, 'MODEL_PACK_CONTRACT_INVALID'), evidence_sha256: checksum(controller.evidence_sha256, 'MODEL_PACK_CONTRACT_INVALID') };
  ...
  const acceptedMlflow = { experiment_id: nonempty(mlflow.experiment_id, 'MODEL_PACK_CONTRACT_INVALID'), run_id: nonempty(mlflow.run_id, 'MODEL_PACK_CONTRACT_INVALID'), registered_model_name: nonempty(mlflow.registered_model_name, 'MODEL_PACK_CONTRACT_INVALID'), registered_model_version: registryVersion, artifact_uri: artifactUri };
...
export function serializeModelPackReleaseInput(release_input: ModelPackReleaseInputV1): Uint8Array { return canonical(validateRelease(release_input)); }
...
export function admitModelPackReleaseStatus(input: Readonly<{ release_status: unknown; expected_package: ModelPackIdentityV1 }>): ModelPackReleaseStatusV1 {
  const call = record(input, ['release_status', 'expected_package'], 'MODEL_PACK_CONTRACT_INVALID'); const value = record(call.release_status, ['schema_version', 'package', 'state', 'controller'], 'MODEL_PACK_CONTRACT_INVALID'); exact(value.schema_version, '1.0', 'MODEL_PACK_CONTRACT_UNSUPPORTED'); const packageValue = identity(value.package, 'MODEL_PACK_IDENTITY_MISMATCH'); const expected = identity(call.expected_package, 'MODEL_PACK_IDENTITY_MISMATCH'); if (!idsEqual(packageValue, expected)) return fail('MODEL_PACK_IDENTITY_MISMATCH'); if (value.state !== 'released' && value.state !== 'revoked') return fail('MODEL_PACK_CONTRACT_INVALID'); const controller = record(value.controller, ['decision_id', 'evidence_id', 'decided_at'], 'MODEL_PACK_CONTRACT_INVALID'); return detached({ schema_version: '1.0', package: packageValue, state: value.state, controller: { decision_id: nonempty(controller.decision_id, 'MODEL_PACK_CONTRACT_INVALID'), evidence_id: nonempty(controller.evidence_id, 'MODEL_PACK_CONTRACT_INVALID'), decided_at: instant(controller.decided_at, 'MODEL_PACK_CONTRACT_INVALID') } }) as ModelPackReleaseStatusV1;
}
...
export function canonicalCategoryDemandInputBytes(input: Readonly<{ admitted_input: CategoryDemandInputV1; manifest: ModelPackManifestV1 }>): Uint8Array { return canonical(admitCategoryDemandInput({ candidate: input.admitted_input, manifest: input.manifest })); }
export function admitCategoryDemandForecast(input: Readonly<{ candidate: unknown; manifest: ModelPackManifestV1; admitted_input: CategoryDemandInputV1 }>): CategoryDemandForecastV1 {
  const call = record(input, ['candidate', 'manifest', 'admitted_input'], 'MODEL_PACK_OUTPUT_INVALID'); const manifest = validateManifest(call.manifest); const admittedInput = admitCategoryDemandInput({ candidate: call.admitted_input, manifest }); const value = record(call.candidate, ['contract_version', 'as_of_date', 'currency', 'predictions'], 'MODEL_PACK_OUTPUT_INVALID'); exact(value.contract_version, '1.0', 'MODEL_PACK_CONTRACT_UNSUPPORTED'); if (value.as_of_date !== admittedInput.as_of_date || value.currency !== admittedInput.currency || !Array.isArray(value.predictions)) return fail('MODEL_PACK_OUTPUT_INVALID'); const rows = value.predictions.map((candidate) => { const row = record(candidate, ['business_date', 'product_category', 'predicted_order_count', 'predicted_net_order_amount', 'order_count_interval_80', 'net_order_amount_interval_80'], 'MODEL_PACK_OUTPUT_INVALID'); const interval = (candidateInterval: unknown): PredictionInterval80V1 => { const intervalValue = record(candidateInterval, ['lower', 'upper'], 'MODEL_PACK_OUTPUT_INVALID'); const lower = decimalString(intervalValue.lower, 'MODEL_PACK_OUTPUT_INVALID'); const upper = decimalString(intervalValue.upper, 'MODEL_PACK_OUTPUT_INVALID'); if (decimalNumber(lower) > decimalNumber(upper)) return fail('MODEL_PACK_OUTPUT_INVALID'); return { lower, upper }; }; return { business_date: date(row.business_date, 'MODEL_PACK_OUTPUT_INVALID'), product_category: nonempty(row.product_category, 'MODEL_PACK_OUTPUT_INVALID'), predicted_order_count: decimalString(row.predicted_order_count, 'MODEL_PACK_OUTPUT_INVALID'), predicted_net_order_amount: decimalString(row.predicted_net_order_amount, 'MODEL_PACK_OUTPUT_INVALID'), order_count_interval_80: interval(row.order_count_interval_80), net_order_amount_interval_80: interval(row.net_order_amount_interval_80) }; });

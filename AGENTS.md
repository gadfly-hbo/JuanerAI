...
JuanerAI is the commercial project family. Xanthil is its first product; existing functionality and reusable foundations remain preserved. The pending Xanthil Desktop and Model Pack development plans were withdrawn by the user on 2026-09-18 and remain void. `docs/planning/README.md` is the current product-planning entry; it points to the approved [Product Development Blueprint v1.2](docs/planning/2026-09-19/juanerai-product-development-blueprint-v1.2.md), JuanerAI's highest product-development execution guideline. JuanerAI Whitepaper v3.3.4 is its core product reference and the commercialization charter; Whitepaper changes enter development only after the user explicitly notifies JuanerAI, approves a versioned Blueprint revision, that revision passes a fresh Product Plan Development-Readiness Gate, and the result is integrated here. Blueprint v1.0/v1.1 remain immutable history.
...
Before proposing a product Change, read Blueprint v1.2 section 8. Record the current product phase and accepted evidence, target user and one complete job, applicable OSM/PIM line and analysis mode, visible endpoint and non-goals, applicable six-plus-two coverage, data/execution and trust boundaries, research-Demo adoption, reused foundations, and the evidence required before selecting a later Change. Choose the smallest unmet user outcome; the Whitepaper capability inventory is not a backlog or execution authorization.

...
import type { MemberRepurchaseMetrics, PlainRecord, SourceDescriptor } from '../../packages/product-core/local-analysis.ts';
import type { AnalysisSource, ConfirmedContract, LocalAnalysisExecution } from '../../packages/ports/local-analysis.ts';

type AdapterError = Error & { code: string };
type CalculationInput = Parameters<LocalAnalysisExecution['calculateMemberRepurchaseMetrics']>[0];
type ValidationInput = Parameters<LocalAnalysisExecution['validateMemberRepurchaseMetrics']>[0];
type RawMetric = { window_id: string; order_count: number; active_member_count: number; repeat_purchaser_count: number };
...
function error(code: string): AdapterError {
  const value: AdapterError = Object.assign(new Error(code), { code });
  value.stack = code;
  return value;
}
...
function frozenPlain(value: unknown): value is PlainRecord { return plain(value) && Object.isFrozen(value); }
function closed(value: unknown, keys: readonly string[], code = 'ANALYSIS_EXECUTION_FAILED'): asserts value is PlainRecord {
...
function contained(root: string, candidate: string): boolean { return candidate.startsWith(`${root}${sep}`); }
...
function safeRoot(config: unknown): string {
  ...
  try {
    ...
    if (errorCode(cause) === 'SOURCE_BOUNDARY_VIOLATION') throw cause;
    reject('SOURCE_BOUNDARY_VIOLATION');
  }
}
function validSource(source: unknown, identityCode = 'FIXTURE_MISMATCH'): asserts source is AnalysisSource {
...
function validRun(run_id: unknown, contract: unknown): void {
  if (typeof run_id !== 'string' || !runIdPattern.test(run_id) || !plain(contract) || contract.run_id !== run_id) reject('ANALYSIS_EXECUTION_FAILED');
}
async function sourceBytes(root: string, source: unknown, mismatchCode = 'FIXTURE_MISMATCH', identityCode = mismatchCode, missingCode = 'FIXTURE_NOT_FOUND'): Promise<{ bytes: Buffer; path: string; read_at: string }> {
  ...
  try {
    ...
    if (typeof code === 'string' && ['SOURCE_BOUNDARY_VIOLATION', 'FIXTURE_MISMATCH', 'SOURCE_CHANGED', 'CONTRACT_VERSION_UNSUPPORTED'].includes(code)) throw cause;
    if (code === 'ENOENT') reject(missingCode);
    reject('SOURCE_BOUNDARY_VIOLATION');
  }
}
function validPreflightInput(input: unknown): asserts input is Parameters<LocalAnalysisExecution['preflightApprovedFixture']>[0] {
...
function frozenIdentity(source: AnalysisSource, byte_size: number, read_at: string): SourceDescriptor {
...
function validateCall(input: CalculationInput | ValidationInput, withSql = false): void {
  const keys = withSql
...
function toResult(values: unknown): MemberRepurchaseMetrics {
  if (!Array.isArray(values)) reject('VALIDATION_FAILED');
  if (!values.every(isRawMetric)) reject('VALIDATION_FAILED');
  const byWindow = new Map(values.map((value) => [value.window_id, value]));
  const metric = (id: string, start_date: string, end_date: string) => {
...
function isRawMetric(value: unknown): value is RawMetric {
  return plain(value) && typeof value.window_id === 'string' && typeof value.order_count === 'number'
    && typeof value.active_member_count === 'number' && typeof value.repeat_purchaser_count === 'number';
}
function run(command: string, args: readonly string[], signal: AbortSignal, seconds: number): Promise<string> {
  return new Promise<string>((resolveResult, rejectResult) => {
    let settled = false;
    const finish = <T>(fn: (value: T) => void, value: T) => { if (!settled) { settled = true; clearTimeout(timer); signal.removeEventListener('abort', onAbort); fn(value); } };
...
function sqlLiteral(path: string): string { return path.replaceAll("'", "''"); }
function sqlValues(output: string): RawMetric[] {
  try {
    const value: unknown = JSON.parse(output);
    if (!Array.isArray(value) || value.length !== 2) reject('VALIDATION_FAILED');
    return value.map((row) => { if (!plain(row) || typeof row.window_id !== 'string') reject('VALIDATION_FAILED'); return { window_id: row.window_id, order_count: Number(row.order_count), active_member_count: Number(row.active_member_count), repeat_purchaser_count: Number(row.repeat_purchaser_count) }; });
  } catch (cause) {
    if (errorCode(cause) === 'VALIDATION_FAILED') throw cause;
    reject('ANALYSIS_EXECUTION_FAILED');
  }
}
function pythonValues(output: string): RawMetric[] {
  try {
    const value: unknown = JSON.parse(output);
    if (!plain(value)) reject('ANALYSIS_EXECUTION_FAILED');
    return ['baseline', 'recent'].map((window_id) => { const metric = value[window_id]; if (!plain(metric)) reject('ANALYSIS_EXECUTION_FAILED'); return { window_id, order_count: Number(metric.order_count), active_member_count: Number(metric.active_member_count), repeat_purchaser_count: Number(metric.repeat_purchaser_count) }; });
  } catch { reject('ANALYSIS_EXECUTION_FAILED'); }
}
function sameResult(value: unknown, expected: unknown): boolean { return JSON.stringify(value) === JSON.stringify(expected); }

export function createDuckDbPythonLocalAnalysisExecution(config: unknown): LocalAnalysisExecution {
  ...
  return Object.freeze({
    ...
    async validateMemberRepurchaseMetrics(input) {
      ...
      const { calculation_kind, ...supplied } = input.sql_result;
      if (calculation_kind !== 'sql' || !sameResult(supplied, expected)) reject('VALIDATION_FAILED');

...
function transportIdentifier(value) {
  return typeof value === 'string' && /^[A-Z][A-Z0-9_]*-[0-9]+$/.test(value);
}

function validateOutputRequirements(value) {
...
function validateDiscoveryContext(value) {
  ...
  if (!exactOrderedObject(source, ['source_id', 'version', 'kind', 'path', 'sha256', 'byte_size', 'columns', 'date_coverage'], { frozen: true })
    ...
    || !Number.isInteger(source.byte_size) || source.byte_size < 1
...
function validateFindingContext(value) {
...
function exactArray(value, expected, { frozen = false } = {}) {
...
function frozenStatus(value, key, expected) {
  return exactObject(value, [key], { frozen: true }) && value[key] === expected;
}

function frozenActiveStatus(value, names) {
  return exactObject(value, ['active_tool_names'], { frozen: true })
    && exactArray(value.active_tool_names, names, { frozen: true });
}

function frozenIdentity(value) { return exactModel(value, { frozen: true }); }

function validateFacade(facade) {
...
function closedTerminalJson(text) {
...
function parseFinalText(event, phase) {
  ...
  try {
    ...
    if (phase === 'discovery' && Object.keys(parsed).length > 0
      && !exactOrderedObject(parsed, ['protocol', 'source', 'comparison', 'delivery'])) return parsed;
    if (phase === 'execution' && exactObject(parsed, ['finding']) && isPlainObject(parsed.finding)) return parsed.finding;
  } catch (error) {
...
function projectEvent(event, rawSdk = false) {
...
function createInertResourceLoader(sdk, settings, systemPrompt) {
...
async function createProductionReadiness(request) {
  let sdk;
  let runtime;
  try {
    sdk = await import('@earendil-works/pi-coding-agent');
    runtime = await sdk.ModelRuntime.create({ allowModelNetwork: false, refreshOnCreate: false });
  ...
  return Object.freeze({ sdk, runtime, model });
}
...
async function createProductionFacade(request, readiness) {
  let listener;
  let disposed = false;
  const { sdk, runtime, model } = readiness;
  ...
  const realized = { session, unsubscribe: session.subscribe?.((event) => {
  ...
  return Object.freeze({
    ...
    dispose() { if (arguments.length !== 0) throw protocol(); if (!disposed) { disposed = true; listener = undefined; realized.unsubscribe?.(); session.dispose?.(); } return Object.freeze({ disposed: true }); },
  });
}

export function createPiAgentAnalysisRuntime(config, injection) {
    ...
    async preflightModel(input) {
      ...
      if (!readinessPromise) {
        readinessPromise = (async () => {
          if (injectedFactory) return Object.freeze({ injected: true });
          try {
            return await createProductionReadiness(readinessRequest);
          } catch (error) {
            if (error?.code === 'MODEL_UNAVAILABLE' || error?.code === 'RUNTIME_UNAVAILABLE') throw error;
            throw sanitized('RUNTIME_UNAVAILABLE');
          }
        })();
      }
    ...
    async openSession(input) {
      ...
      const tools = TOOL_NAMES.map((name, index) => deepFreeze({
        ...
        async execute(toolCallId, params, signal) {
          if (phase === 'cancelled' || signal?.aborted) throw sanitized('CANCELLED');
          ...
          try {
            ...
            if (!admissionOpen || phase === 'cancelled' || signal?.aborted) throw sanitized('CANCELLED');
            ...
            if (error?.code === 'TOOL_POLICY_VIOLATION') throw error;
            if (error?.code === 'PROTOCOL_FAILURE') throw error;
      ...
      const cleanup = async () => {
        ...
        cleanupPromise = (async () => {
          ...
          try { if (!frozenStatus(await facade.abort(), 'aborted', true)) bad = true; } catch { bad = true; }
          try { if (!frozenStatus(await facade.waitForIdle(), 'idle', true)) bad = true; } catch { bad = true; }
          try { if (!frozenStatus(facade.dispose(), 'disposed', true)) bad = true; } catch { bad = true; }
      ...
      const fail = async (code) => {
        if (phase === 'cancelled') code = 'CANCELLED';
        failureCode = failureCode ?? code;
        phase = failureCode === 'CANCELLED' ? 'cancelled' : 'failed';
        await cleanup();
        throw sanitized(failureCode);
      };
      const receive = (rawEvent) => {
        if (!admissionOpen) {
          if (phase !== 'cancelled') failureCode ??= 'PROTOCOL_FAILURE';
          return undefined;
        }
        try {
          ...
          if (event.type === 'message_update') {
            ...
            if (currentTurn.kind !== 'execution' || currentTurn.terminalStage !== 0 || !exactObject(event, ['type', 'toolCallId', 'toolName', 'result', 'isError'])
              || event.isError !== false) { failureCode ??= 'TOOL_POLICY_VIOLATION'; return undefined; }
        ...
        } catch (error) { failureCode ??= error?.code === 'TOOL_POLICY_VIOLATION' ? 'TOOL_POLICY_VIOLATION' : 'PROTOCOL_FAILURE'; }
        return undefined;
      };

      try {
        facade = validateFacade(await (injectedFactory ? injectedFactory(request) : createProductionFacade(request, readiness)));
        unsubscribe = facade.subscribe(receive);
        ...
        if (error?.code === 'PROTOCOL_FAILURE') throw protocol();
        throw sanitized('MODEL_UNAVAILABLE');
      }

      const session = Object.freeze({
        async discover(value) {
          if (phase === 'cancelled') throw sanitized('CANCELLED');
          ...
          try {
            const active = facade.setActiveTools(Object.freeze([]));
            if (!frozenActiveStatus(active, [])) throw protocol();
            const pending = facade.prompt(`XANTHIL_DISCOVERY_V1\n${JSON.stringify({
              phase: 'discovery',
              discovery_context,
              required_response: {
            ...
            const actual = facade.getActualModel();
            ...
            return fail(phase === 'cancelled' || error?.code === 'CANCELLED' ? 'CANCELLED' : error?.code === 'MODEL_EXECUTION_FAILED' ? 'MODEL_EXECUTION_FAILED'
              : failureCode ?? (error?.code ? error.code : 'MODEL_EXECUTION_FAILED'));
          }
        },
        async execute(value) {
          if (phase === 'cancelled') throw sanitized('CANCELLED');
          ...
          try {
            const active = facade.setActiveTools(Object.freeze([...TOOL_NAMES]));
            if (!frozenActiveStatus(active, TOOL_NAMES)) throw protocol();
            const pending = facade.prompt(`XANTHIL_EXECUTION_V1\n${JSON.stringify({
              phase: 'execution',
            ...
            const code = phase === 'cancelled' || error?.code === 'CANCELLED' ? 'CANCELLED' : error?.code === 'MODEL_EXECUTION_FAILED' ? 'MODEL_EXECUTION_FAILED'
              : failureCode ?? (error?.code ? error.code : 'MODEL_EXECUTION_FAILED');
            return fail(code);
          }
        },
        async cancel() {
          ...
          if (phase === 'cancelled') return deepFreeze({ cancelled: true, was_confirmed: false });
